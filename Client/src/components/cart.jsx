    ...
    <div className='flex w-full bg-gray-50 overflow-x-hidden'>
      ...
      {cartItems.length === 0 ? (
        ...
        <div id='main' className='flex flex-col   '>
          <div className='flex  w-[100vw]  !p-3 md:!p-16 flex-col '>
            ...
            <div className='flex font-semibold text-gray-900 mb-4 text-md fontp pt-2 bg-white justify-around items-center shadow-lg  min-h-16'>
              <p className='flex-1 text-center text-xs md:text-lg '>Product</p>
              <p className='flex-1 text-center text-xs md:text-lg '>Price</p>
              <p className='flex-1 text-center text-xs md:text-lg '>Quantity</p>
              <p className='flex-1 text-center text-xs md:text-lg '>Sub Total</p>
                  ...
                  <div className='flex font-semibold  mb-2 text-gray-900 text-md fontp p-3 bg-white justify-around items-center shadow-lg w-full min-h-16'>
                    <div className='flex-1 flex-col'>
                      <div className='flex items-center mb-2'>
                        <img src={item.img} alt={item.name} className="w-10 hidden md:flex h-auto mr-2" />
                        <p className="md:!text-md !text-xs font-semibold">{item.name}</p>
                      </div>
                      {item.variations && Object.keys(item.variations).length > 0 && (
                        <div className="flex mt-1 md:pl-11">
                          {Object.keys(item.variations).map(variationId => (
                            <span className='md:!text-sm !text-xs pr-2 text-gray-500' key={variationId}>
                    ...
                    <div className='flex-1 text-center'>
                      <p className="md:!text-lg !text-xs">{item.currency}{item.price}</p>
                    </div>
                    <div className='flex-1 pb-3 md:pb-0  text-center md:pl-2 rm '>
                      <button onClick={() => handleQuantityChange(item, item.quantity - 1)} className='md:w-10 w-5 hover:bg-red-500 hover:text-white border-2 border-gray-700'>-</button>
                      <input type="number" min="1" value={item.quantity} onChange={(e) => handleQuantityChange(item, parseInt(e.target.value))} className=" md:w-14 w-5  text-md border-t-2 border-b-2 text-center border-gray-700 " />
                      <button onClick={() => handleQuantityChange(item, item.quantity + 1)} className='md:w-10 w-5 hover:bg-red-500 hover:text-white border-2 border-gray-700'>+</button>
                    </div>
                    <div className='flex-1 text-center'>
                      <div className='flex justify-center md:!text-lg !text-xs items-center space-x-2'>
          ...
          <div className='md:!pl-16 !pl-3 mb-8  flex flex-col md:flex-row justify-between pr-28 items-start'>
            ...
            <div className="flex justify-center gap-4 items-center">
              <input type="text" placeholder='Coupon Code' className='md:min-w-[30-vw] min-w-[20vw] pl-3 h-[8vh] mt-2 border-2 border-gray-500 rounded-md' />
              <p className='bg-red-500 mt-4 rounded-md h-[8vh] text-xs md:!text-sm  text-nowrap text-white pl-3 pr-3  md:pb-2  !pt-3'>Apply Coupon</p>
            </div>
            </div>
            <div className="totoldata md:w-[40%] w-[100%] border-2 p-4 border-gray-800">
